...
import { remindersSentTotal, notificationFailuresTotal, notificationLatency } from '@/shared/utils/metrics'
  ...
  async (job: Job<JobData>) => {
    ...
    try {
      if (job.name === 'dose-reminder') {
        logger.info(`[WORKER] Sending reminder for ${medicationName} (Dose ID: ${doseId})`)
        console.log(`\n🔔 REMINDER: Time to take your medication: ${medicationName}!`)
        remindersSentTotal.add(1)
      }

      if (job.name === 'check-missed-dose') {
        logger.info(`[WORKER] Checking if dose ${doseId} for ${medicationName} was missed`)
